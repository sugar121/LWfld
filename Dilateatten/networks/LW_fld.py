...
from fld_utils import *



class LW_fld(nn.Module):
    def __init__(self, in_chans=3, depths=[3, 3, 9, 3], dims=[24, 48, 88, 168],
                 ...
                 use_pos_embd_global=False, d2_scales=[2, 3, 4, 5], transformer_depth=[2,3,4],**kwargs):
        super().__init__()
        for g in global_block_type:
            assert g in ['None', 'SDTA','DA']
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                if j > depths[i] - global_block[i] - 1:
                    if global_block_type[i] == 'CAM':
                        stage_blocks.append(CAM(dim=dims[i], drop_path=dp_rates[cur + j],
                                                        expan_ratio=expan_ratio, scales=d2_scales[i],
                                                        use_pos_emb=use_pos_embd_xca[i], num_heads=heads[i]))
                    elif global_block_type[i] == 'DDAM':
                        stage_blocks.append(DDAM(dim=dims[i], depth=transformer_depth[i - 1], channel=dims[i], kernel_size=3))
                    else:
